...
namespace DuiLib
{
	...
	void CContainerUI::SetScrollStepLength(int iStep)
	{
		iDefaultStep = iStep;
	}

	int	 CContainerUI::GetScrollStepLength()
	{
		return iDefaultStep;
	}
	...
	{
		int cyLine = 8;
		int iStepLength = iDefaultStep;
		if ( iStepLength == 0 )	// if user not set step length ,that is default length
		{
			iStepLength = 8;
		}

		if( m_pManager ) cyLine = m_pManager->GetDefaultFontInfo()->tm.tmHeight + iDefaultStep;
	...
	void CContainerUI::LineDown()
	{
		...
		if( m_pManager ) 
			cyLine = m_pManager->GetDefaultFontInfo()->tm.tmHeight + iDefaultStep;
	...
	void CContainerUI::EnableScrollBar(bool bEnableVertical, bool bEnableHorizontal)
	{
		if( bEnableVertical && !m_pVerticalScrollBar ) {
			...
			m_pVerticalScrollBar->SetVisible(false);    // 让滚动条的初始状态为隐藏状态
			if ( m_pManager ) {
		...
		if( bEnableHorizontal && !m_pHorizontalScrollBar ) {
			...
			m_pHorizontalScrollBar->SetVisible(false);    // 让滚动条的初始状态为隐藏状态
			if ( m_pManager ) {
