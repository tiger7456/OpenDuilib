...
//#endif
    ...
    case WM_PAINT:
        ...
        return true;
		// 2013-07-31 修改，支持透明的异性窗体，modify by shaoyuan1943@gmail.com
		/*{
			// Should we paint?
			RECT rcPaint = { 0 };
			if( !::GetUpdateRect(m_hWndPaint, &rcPaint, FALSE) ) return true;
...
// 2013-10-22 添加，查询计时器状态是否激活
/*
	如果该计时器不存在则直接返回false，如果存在检测其状态是否被Killed
*/
bool CPaintManagerUI::IsTimerActive(CControlUI* pControl, UINT nTimerID)
{
	ASSERT(pControl!=NULL);
	ASSERT( nTimerID > 0);
	for( int i = 0; i< m_aTimers.GetSize(); i++ ) 
