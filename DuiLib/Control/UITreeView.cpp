...
namespace DuiLib
{
	...
	void CTreeNodeUI::DoEvent( TEventUI& event )
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
		{
			if ( IsEnabled() )
			{
				m_pManager->SendNotify(this,__T("nodeclick"));
				Invalidate();
			}
			return ;
		}
	...
	bool CTreeViewUI::OnFolderChanged( void* param )
	{
		...
		if(pMsg->sType == _T("selectchanged"))
		{
			...
			else	// 2013-09-12修改，在节点关闭时发出通知，以便做其他的事情
			{
				m_pManager->SendNotify(pItem,__T("itemshrink"));
			}
			SetItemExpand(pFolder->GetCheck(),pItem);
		...
		if(_TreeNode)
		{
			if(_TreeNode->GetCountChild() > 0)
			{
				...
				for(int nIndex = 0;nIndex < nCount;nIndex++)
				{
					...
					// 2013-09-04,原句：if(pItem->GetCountChild() && !pItem->GetFolderButton()->IsSelected()) 
